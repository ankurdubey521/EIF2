...
const deadManCheckStillAlive = async () => {
  ...
  const hash = (await await contract.checkIfStillAliveWasCalledRecently()).hash;
  console.log(`checkIfStillAliveWasCalledRecently() tx hash: ${hash}`);
};

deadManCheckStillAlive()
  .then(() => process.exit(0))
  .catch((error) => {
    console.error(error);
    process.exit(1);
